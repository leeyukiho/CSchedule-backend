...
export class AuthService {
  ...
  async submitLogin(
    ...
    const binding = await this.prisma.userSchoolBinding.create({
      data: {
        userId: user.id,
        schoolId,
        providerId: existingSchool.providerId ?? schoolId,
        studentNoEncrypted: input.username ? `masked:${input.username}` : undefined,
        status: 'need_login',
        authState: {
    ...
    try {
      const cache = await this.courseSync.fetchAndCacheByCredentials({
        ...
        semesterId: typeof input.extra?.semesterId === 'string' ? input.extra.semesterId : undefined,
      })
  ...
  async importSession(
    ...
    const binding = input.bindingId
      ...
      : await this.createWebviewBinding(schoolId, school.providerId ?? schoolId, input.contextId)
    ...
    return {
      ...
      bindingId: binding.id,
      requiredFetchTargets: ['course'],
      message: 'Session import is not available for this provider. Fetch data inside WebView and upload raw-data.',
    }
  }
